...
*/
 ...
 Shader "Custom/LogoText" 
 {
	Properties 
	{
		_BaseTex ("Base (RGB)", 2D) = "white" {}
		_SineScale ("Sine Scale",float) = 0.0
...
#pragma exclude_renderers xbox360
			...
			{
				half2 uv = i.uv;
				uv.y += sin((_Time * 30.0) + (i.uv.x * 10.0)) * _SineScale;
				half4 base = tex2D (_BaseTex, uv);

				half4 vButton = half4(0.0,0.2,0.0,0.0);
				vButton *= _Button;

				half4 vOver = half4(1.0,0.0,0.0,0.0);
				vOver *= _Over;

				vButton += vOver;

				return (base + vButton) * _Alpha; 
			}
